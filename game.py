...
def resetGame():
	global surface_pos,player_color,player_health,player_score
	...
	while True:
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN:
				# The user pressed a key to start playing
				return
		...
		text = font.render("The Endless Battle",1,Colors["red"])
		textRect = text.get_rect()
		textRect.center = (window_width / 2,(window_height / 2) - 80)
		window.blit(text,textRect)

		# Displays the player
		player_rect = player_surface.get_rect(center = window_center)
		window.blit(player_surface,player_rect.topleft)
		points = ((surface_width / 2,0),(0,surface_height),(surface_width,surface_height))
		pygame.draw.polygon(player_surface,player_color,points)

		font = pygame.font.Font(None,40)
		text = font.render("Press any key to start this futile battle",1,Colors["white"])
		textRect = text.get_rect()
		textRect.center = (window_width / 2,(window_height / 2) + 80)
		window.blit(text,textRect)
...
while not exit_game:
	...
	# Displays the game over screen when the player's health reaches 0
	if player_health <= 0:
		gameOver()
